...
The repository is implementing **Phase 1B: Production GitHub App Integration**. Hosted Supabase is current through migration `026`, with a verified zero-mismatch ACL matrix. Owner `surgeservicesllc@gmail.com` is authenticated, and primary installation `153445938` remains connected to exactly `surgeservicesllc/SoftwareFactory`. Connections, Projects, Files, Activity, ordinary draft PR `#6`, owner-approved protected RED draft PR `#7`, likely-secret rejection, commit identity, and immutable evidence pass on production release `0bd0485`. Primary App `4573846` still cannot retain its webhook under OPEN GitHub Support ticket [#4660724](https://support.github.com/ticket/personal/0/4660724). Owner-only candidate App `4582606` (`surge-softwarefactory-next`) now retains the exact active webhook and has isolated Sensitive Production/Preview configuration. The dual-App and atomic reversible handoff code plus migration `027` passes the full local check at 56 files/436 tests and a 38-route build, but is not committed, deployed, hosted, installed, or handed off; no signed processed candidate delivery exists. The webhook and candidate therefore remain **Not Connected**, and Phase 1B acceptance is incomplete. The currently selected local Supabase CLI profile is unauthorized or associated with the wrong account for a fresh hosted recheck; it was not used for any mutation.

Production UI: [https://softwarefactory-tan.vercel.app](https://softwarefactory-tan.vercel.app), in Vercel project `surgeservices-projects/softwarefactory` (`prj_pAsrhftaVWI4SyaqstgRVSWHJkdD`). Current production is READY deployment `dpl_AEirYPnCrKemJjiFX7bKGc7626jX`, immutable URL `https://softwarefactory-fa4gc8jfm-surgeservices-projects.vercel.app`, sourced from exact `main` application commit `0bd048565a9e002848c5553ccbe43ab0e217780e` and served by the stable alias. CI run `31704289754` passed both jobs. No secret values are recorded here. See [`AI/CURRENT_STATE.md`](AI/CURRENT_STATE.md) for the remaining Phase 1B checks.
...
- The pre-release replacement path keeps primary and candidate GitHub Apps cryptographically isolated, binds installation state and repository tokens to the exact App ID, validates webhook signing-App provenance, and blocks owner handoff until the target installation has a processed signed delivery. Migration `027` preserves the existing project/history and supports an evidence-bound reverse handoff while both installations remain active; it is not hosted yet.
