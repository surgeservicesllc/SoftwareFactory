...
| API/rate limits | **PARTIAL** | The per-provider concurrency limit bounds simultaneous calls, which is what actually trips most provider-side limits. There is still no accounting of requests or tokens *per unit time*, so a burst under the concurrency cap is not detected. |
