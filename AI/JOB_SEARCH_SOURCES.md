...
| LinkedIn Jobs | Link-out (person's own browser) | LinkedIn's terms prohibit automated collection, and this repository has twice declined to scrape it; that decision stands. The link opens LinkedIn's own job search in your browser, which is the permitted path. |
| Indeed | Link-out (person's own browser) | Indeed's publisher API is closed to new partners and scraping is prohibited, so the link opens Indeed's own search in your browser. |
...
| Filters (keywords AND/OR, exclude keywords/companies, work model, title-derived seniority, salary minimum, only-with-salary, date posted, source picker, minimum match score) with chips + Clear All | `unify.ts` `applyUnifiedFilters` shared by route, panel and alert engine; seniority via `deriveSeniority` — read from the job title alone, labeled that way, never invented | Route tests + panel tests (29) + `deriveSeniority` unit tests; location radius and industry facets are still not offered because the connected boards expose neither as data — nothing pretends otherwise |
