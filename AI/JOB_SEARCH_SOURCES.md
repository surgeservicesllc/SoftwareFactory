...
| Filters (keywords AND/OR, exclude keywords/companies, work model, salary minimum, only-with-salary, date posted, source picker, minimum match score) with chips + Clear All | `unify.ts` `applyUnifiedFilters` shared by route and panel; source picker over the catalogue | Route tests (20) + panel tests (22); location radius and seniority/industry facets are not offered because the connected boards do not expose them as data — nothing pretends otherwise |
