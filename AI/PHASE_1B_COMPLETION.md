...
| 8 | Detect stale SHA / version conflicts | **PASS** | CODE: `app/api/github/repositories/[owner]/[repo]/changes/route.ts:370` refuses with `409 stale_file` when the live blob SHA differs from the submitted `expectedBlobSha`; the reservation stores that SHA (`reserve_github_change_request`). TEST: `tests/unit/github-stale-file-refusal.test.ts` (5 tests). **Evidence corrected 2026-08-15:** this row previously cited `tests/unit/github-protected-change-route.test.ts`, which tests commit attribution, the approval requirement, administrator rejection and the owner-approved draft — none of which touch the stale path. Searching the whole tree for `stale_file` returned nothing, so the item had been scored on code that existed rather than a test that ran. The new suite asserts the refusal, that the expected SHA is required rather than defaulted (a defaulted one makes the comparison vacuous), that the check runs **before** branch creation and the commit, that the SHA is also handed to the commit so GitHub re-checks it, and that a stale save is distinguished from an unchanged one. Mutation-tested by moving the check after branch creation: the ordering test fails. |
