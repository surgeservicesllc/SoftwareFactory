...
| **Contact hygiene report** | HubSpot | **HAVE** (ADR-234) — every contact the book should not trust, with the reasons (unreachable, undeliverable, duplicate email, inactive account, untouched for a year), most flagged first; it deletes nothing, and each row opens the account where the audited merge and the preference record already are. |
...
| **QuickBooks sync** | Briostack, GorillaDesk, Jobber | **PARTIAL** — a balanced general-journal export (ADR-220) ships: invoices raised, payments taken, refunds given and uncollectible invoices written off, every entry balancing by construction and the whole file checked before download. It is an EXPORT a bookkeeper posts, not a sync; the API sync still needs an Intuit account. |
...
| **Forecast scenarios with the owner's own assumptions** | HubSpot | **HAVE** (ADR-234) — annual churn and growth in basis points, one row per workspace with a note on their provenance, compounded month by month beside the recorded forecast with the factor printed; a what-if from the query is applied and never saved; the seed leaves the table empty. |
...
| **Knowledge base, staff and customer** | HubSpot Service Hub (gated) | **HAVE** (ADR-237) — `crm_kb_articles` under forced RLS with a shaped slug, an audience and a published moment; `crm_kb_search` scores title ×3 + body ×1 on whole words with a plural s optional and prints the rank; `crm_portal_articles` is the customer's definer read of published customer articles in their own workspace. Copilot: "What do we tell customers about ants?". |
| **Calendar file for a booked visit** | HubSpot meetings | **HAVE** (ADR-237) — `crm_portal_visit_calendar` (definer, own account only) and the staff work-order route hand over an RFC 5545 file built from the visit's row: UTC moments, escaped text, 75-octet folding, an assumed hour said in the entry. No provider, nothing to connect. |
| **Conditional questions on forms** | PestPac (complaint 13), WorkWave Forms | **HAVE** (ADR-238) — `crm_form_fields.depends_on_field_id` + `show_when` (answered / is_true / is_false / equals / any_of), earlier-question-only so a cycle is impossible, op checked against the parent's type; `crm_form_question_asked` walks the chain; completion counts only asked required questions; an answer to an unasked question is refused; a hidden answer is kept, not deleted. The rule is printed beside the question. |
| **Service-type-triggered forms** | WorkWave Forms (paid tier) | **HAVE** (ADR-238) — `crm_form_templates.trigger_service_types`; an AFTER INSERT trigger on the visit assigns each active matching template once, carrying account, site, visit and technician. |
| **Conversation routing with the reason printed** | HubSpot Service Hub (rule-based routing) | **HAVE** (ADR-240) — no rule table: `crm_request_suggested_assignee` walks branch manager of the address's territory → the territory's rep → the least-loaded active CSR or dispatcher → nobody, and returns the step that chose in a sentence with the load; `crm_request_assign` records the assignment on the account's timeline by name; `crm_request_queue` lists open requests unassigned-first with their suggestion; "mine" is the caller's own staff record. Copilot: "Which requests have nobody assigned?". |
| **Bulk visit edit with per-row outcomes** | PestBoss (bulk scheduling) | **HAVE** (ADR-239) — `crm_work_orders_bulk_edit` reassigns, shifts by days and re-statuses up to 200 visits in one call as the caller; each row answers applied or refused with the reason in words (completed; on a named route for a date; not found). Completion is never set in bulk. |
| **Multi-day route projects** | PestBoss | **HAVE** (ADR-239) — `crm_projects` (a span of at most 31 days, a daily window, weekends optional) and `crm_project_create` writing the project and one real visit per working day in one call; `crm_project_progress` counts days, completed, cancelled, remaining and the next day live; `crm_project_cancel` cancels what is not done and the outcome trigger records each. "Day 2 of 4" beside the visit. |
| **Chemical-lot labels** | PestPac | **HAVE** (ADR-237) — Code 39 labels for lots still in stock, product and expiry on the label, printed from the browser; a lot number the symbology cannot carry prints as text with the reason rather than an uppercased symbol that would scan as a different lot. |
