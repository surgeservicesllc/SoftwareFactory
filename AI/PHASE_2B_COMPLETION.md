...
| 33 | Bot Manager can launch a real graph goal | **PARTIAL** | **Built 2026-08-15.** `POST /api/graphs` compiles a template through `buildLaunchPlan` and calls `create_graph_from_plan` as an authenticated manager. `graph-launch-plan.test.ts` proves the payload against the real function on real PostgreSQL: all **13** registered templates write cleanly, the compiler's topology and risk are what the database ends up holding, and a project belonging to another organization is refused. What is not done is a console control that calls it, and no graph has been created in hosted. The route creates a graph and deliberately does **not** start a run — no executor is wired to the runner, so a started run would sit at PENDING and read as work in progress. |
