...
| 30 | Failure supports bounded retry/reassignment/provider fallback | **PARTIAL** | Retry and one-attempt fallback are built and unit-tested (`lib/providers/runtime.ts`). The reason this stayed partial — only one provider was reachable zero-token — is now addressed in code: `lib/providers/codex-cli-transport.ts` reaches Codex on the ChatGPT subscription, reusing Phase 1C's `resolveCodexAuth` rather than restating the cost policy. Nine unit tests cover it, including that no billing credential enters the child process in subscription mode and that an exhausted usage ceiling reports as `rate_limited` rather than an outage. The **live** fallback demonstration is `BLOCKED_BY_CODEX_QUOTA` until 2026-08-20. |
...
| 33 | Bot Manager can launch a real graph goal | **PASS** | **Built 2026-08-15**, after this row was corrected from a PARTIAL that rested on "the surface exists" when it did not. `POST /api/graphs` compiles a template through `buildLaunchPlan` and calls `create_graph_from_plan` as an authenticated manager; `GraphLaunchControl` on `/solutions/workflows` calls the route. Proven against the real function on real PostgreSQL in `graph-launch-plan.test.ts`: all **13** templates write cleanly, the compiler's topology and risk are what the database holds, and a project in another organization is refused. `graph-launch-control.test.tsx` asserts the surface says **Recorded** and never *running* or *started*. It records a plan and deliberately does not start a run — no executor is wired to the runner, so a started run would sit at PENDING and read as work in progress. |
