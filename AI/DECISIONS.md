...
- Date: 2026-08-23
- Status: Superseded by ADR-137. Its premise — that nothing produces a node in DISCOVER, EVALUATE or DECIDE — held when it was written and no longer holds. Its reasoning against empty scaffolding is upheld, not overturned.
...
## ADR-137 - The enum grows to ten, because eight real nodes now stand in the three stages ADR-136 called unpopulatable

- Date: 2026-08-23
- Status: Accepted
- Supersedes: ADR-136
- Decision: `sdlc_stage` becomes the ten the goal document names — REQUIREMENT, DISCOVER, EVALUATE, DECIDE, ARCHITECT, BUILD, REVIEW, TEST, DEPLOY, MONITOR. `20260823000900_ten_stage_lifecycle.sql` rebuilds the type rather than extending it, because `PRD` and `GOAL` must *merge* into `REQUIREMENT` and a merge is not a rename: both columns are carried through `text`, mapped, and carried back. `agentic_sdlc` becomes 19 nodes across all ten stages, each stage handing the next a typed, versioned package (`lib/sdlc/artifacts.ts`, `STAGE_ARTIFACT_VERSION`).
- Rationale: ADR-136 did not argue that ten stages are wrong. It argued that three of them were empty — "a vocabulary the database can express and nothing can populate — a stage filter that is permanently empty" — and refused to widen an enum ahead of a producer. That reasoning is correct and is why this change ships the producer and the vocabulary in one commit rather than the enum first. The producer now exists: `DISCOVER` holds `discover_internal`, `discover_packages`, `discover_services` and `discover_shortlist`; `EVALUATE` holds `evaluate_fit`, `evaluate_risk` and `evaluate_matrix`; `DECIDE` holds `decide`. Eight nodes, none of the three filters empty. ADR-136 also named its own exit condition — "the prerequisite is a capability that produces them, not an enum value" — and these nodes are that producer.
- On executors: the three DISCOVER search nodes are `ANCHOR`, not `MODEL`, and that is load-bearing rather than incidental. `anchorsFor` counts only what an anchor actually observed, so a DISCOVER stage assembled from `MODEL` nodes could never satisfy the stage's own evidence requirement — the stage would read as researched while nothing had been read. Making the stage's evidence rule unsatisfiable by fabrication is the mechanism that keeps "does this already exist" an answer rather than a guess.
- On migration order: `20260823000700` backfills `graph_nodes.lifecycle_stage` and was applied to production before this change (workflow run 32660207022). It writes `IMPLEMENTATION`, `ARCHITECTURE` and `PRD` — three labels that do not survive this rebuild. `20260823000900` must therefore stay numbered *above* `20260823000700`. Renumbering it below would run the rebuild first and leave the backfill writing labels the type no longer contains. The dependency is recorded in the migration's own header and asserted by `tests/unit/graph-stage-mapping-agreement.test.ts` so a future renumbering trips a test rather than production.
- Consequence: the mapping ADR-136 recorded in prose is now enforced by the migration's `case` expression, so the two cannot drift. Per-stage pages are built against `SDLC_STAGES` (ten). A stage that later loses its last producing node should lose its page before it loses its enum value — the empty-filter harm ADR-136 named is real, and this decision rests entirely on that condition not holding.
