...
## ADR-120 - A privilege gate must name every role, not the roles the migration thought of

- Date: 2026-08-22
- Status: Accepted
- Decision: `20260822001200` revokes EXECUTE on `clear_backlog_tasks` and `clear_all_pipelines` from PUBLIC, anon, authenticated and service_role, then grants it back to authenticated alone, with a preflight on the known input and a postflight asserting the ACL literally holds two entries — owner and authenticated — and nothing else. The `scope=clear-controls` readback and `scope=probe` both now ask `service_role` as well as `authenticated` and `anon`.
- Rationale: this is the second time the same sentence shipped a hole. `20260822000500` ended `revoke all on function ... from public, anon` and ADR-118 contained the residual `service_role` grant that left. `20260822000800` was written the same way, four days later, and left the same grant on two SECURITY DEFINER functions that delete rows. Both migrations passed their own post-apply gate, because in each case the gate asked about exactly the roles the migration had remembered to revoke — an assertion derived from the same incomplete thought as the code it was checking. Hosted probe run `32590061431` measured `service_may_execute = t` on both clears; nothing local could have, because PGlite has no default privileges configured for these roles.
- Severity, stated exactly rather than dramatised: `can_manage_organization` resolves through `has_organization_role`, which requires `auth.uid() is not null`, so a raw `service_role` connection carrying no JWT is refused inside the function body even holding EXECUTE. The exposure was not "service_role can clear any workspace today". It was that the only thing standing between an RLS-bypassing role and a cascading delete was one `auth.uid()` test in the body, where the design had also intended a grant boundary — and `20260822000800`'s own comment claimed "callable by members only" of a database where that was untrue.
- Consequence: `tests/integration/clear-control-acl-contract.behavior.test.ts` reproduces the measured hosted state rather than the CREATE-FUNCTION-time state — `anon` false, `service_role` true, which is what `00800`'s own revoke left — and proves the contraction closes it, is replayable, re-closes a grant handed back afterwards, and refuses a drifted input. It is mutation-checked: narrowing the revoke back to `public, anon` fails two cases. The postflight counts ACL entries rather than checking for the absence of `service_role`, because a grant to a role nobody thought to name is the entire failure mode and asking about a named role reproduces the original mistake.
- Also: the probe that found this had a defect of its own — it selected `label` from `pg_enum`, whose column is `enumlabel`, so the step exited 1 after printing every read. The reads it had already printed are what made the finding, which is an argument for a diagnostic step that prints as it goes rather than accumulating and asserting at the end.
