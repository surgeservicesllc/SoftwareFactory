...
- Date: 2026-08-14
- Status: Accepted locally; migration `20260814000200` is unhosted
...
## ADR-058 - The database decides whether a claim is anchored

- Date: 2026-08-14
- Status: Accepted locally; migration `20260814000700` is unhosted
- Decision: `record_claim_anchoring` receives a node run, a claim, and a set of anchor IDs — never a verdict. It resolves each anchor, discards any belonging to another organization or another graph run, joins against `claim_acceptable_anchors` to keep only kinds that can support that claim, and then decides: contradicting evidence refutes, state-only evidence (`passed is null`) cannot support a success claim, and silence is a refusal. The computed `anchored` flag and a stored reason are written to `node_run_claims`, and only the supporting anchors are linked through `claim_anchors`. `graph_anchors` has no provider or model column, `observed_at` records when the world was seen rather than when the row was written, and a future-dated observation is refused by check constraint. `(node_run_id, claim)` is unique. `lib/graph/anchor-store.ts` is a thin server-only client that sends evidence and returns the database's answer.
- Rationale: `lib/graph/anchors.ts` already modelled the rule that a worker saying "the tests pass" is a hypothesis while the test result is evidence, but a check performed in memory and discarded leaves the next reader nothing to inspect. Putting the acceptable-kind mapping in SQL duplicates a TypeScript constant, which is a real cost — but the alternative is letting the caller pass in what counts as acceptable evidence, and a caller able to define that could anchor any claim with anything. This is a security boundary, so the authority belongs on the far side of it. The duplication is paid for by a drift test that reads both and fails if they disagree; that test was verified against a deliberately removed mapping row rather than assumed to work.
- Consequence: an agent cannot record that its own claim is supported; it can only offer observations and be told. Refuted claims are durable and carry their reason, so a later reader sees why rather than re-deriving it, and the contradicting observations survive even though they are not linked as support. The uniqueness constraint means a refused claim cannot be quietly re-attempted until it passes — a genuine retry needs a new node run, which is the honest representation of "we did the work again". `graph_verifications.anchor_count` makes "this verifier reasoned without evidence" a query rather than an inference. Nothing calls this yet: no provider credential exists, so no node has produced a real observation, and the tables are empty by absence of runs rather than by absence of findings.
