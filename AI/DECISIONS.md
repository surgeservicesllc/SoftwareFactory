...
## ADR-041 - Auth failures name a cause the caller can act on

- Date: 2026-08-13
- Status: Accepted
- Decision: Every Supabase Auth failure passes through `lib/supabase/auth-errors.ts`, which maps a provider code to a fixed status, code, and message plus two flags: `canResend` (another email would help) and `ownerActionRequired` (no caller input will help). Provider error text is never echoed to the browser; the provider's message is read only to recognize an undeliverable confirmation email, which Supabase reports as a generic `unexpected_failure`. `email_not_confirmed` is `403` and distinct from `401 invalid_credentials`. Account-existence disclosure stays bounded by call context: `user_not_found` is answerable for a self-service resend, never for a magic link. `POST /api/auth/resend-confirmation` returns the same neutral `202` whether or not an unconfirmed account exists. Only an address-independent failure may short-circuit that response, which in practice means `email_provider_disabled` alone: Supabase attempts a send — and so consumes mail quota, or fails delivery — only for an address that has an account awaiting confirmation, so reporting a rate limit or a delivery failure there would answer the question the neutral response exists to refuse. Live probing confirmed this before the branch was narrowed: a real unconfirmed account returned `429` while an invented address returned `202`. The delivery caveat is instead stated unconditionally in the `202` body, where it carries no signal.
- Consequence: The routes no longer flatten distinct failures into one sentence. Sign-in previously reported an unconfirmed address as a rejected password, which sent people to reset a password that was already correct and left the account unreachable, since no resend path existed. Confirmation email delivery is now an explicit, named product dependency: with `enable_confirmations = true` and no custom SMTP, Supabase's built-in mail service is limited to a few messages per hour, and that limit — not the application — is what stops new accounts from becoming usable. Adding an auth route means routing its failures through the same translation; returning a raw provider message anywhere re-opens the leak this closes.

## ADR-042 - The site renders differently for a verified viewer

- Date: 2026-08-13
- Status: Accepted
- Decision: Every route-group layout resolves a `Viewer` on the server through `lib/auth/viewer.ts` and passes it into the header and shell, so the signed-in navigation is present in the first server render rather than appearing after hydration. The viewer comes from `auth.getUser()`; cookie contents never produce a signed-in viewer, and `readViewer` never throws — an unconfigured or unreachable Supabase resolves to signed-out so the public marketing site still renders. `lib/navigation.ts` derives the entries and is deliberately not `server-only`, because it holds link labels rather than secrets and is shared by server layouts and the client header. Super administrators are configured by the server-only `SUPER_ADMIN_EMAILS` variable, which replaces the documented default list rather than extending it, and the role is granted only when the provider reports the address as confirmed. `/admin` re-checks the role on the server on every request.
- Consequence: Showing a navigation entry is presentation, never authorization; each destination still enforces its own access, and a hidden link is not a control. Reading the session makes the marketing pages dynamic — they stay public and unchanged for anonymous visitors, but they are no longer statically prerendered. The confirmed-email requirement couples the super-administrator role to Supabase's email confirmation setting: turning on `mailer_autoconfirm` would let an unverified sign-up claim a listed address, so that setting and this role must be changed together or not at all.
