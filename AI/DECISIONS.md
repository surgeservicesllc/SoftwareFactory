...
## ADR-032 - A database refusal is part of the truthful status contract

- Date: 2026-08-14
- Status: Accepted; covered by unit tests, live confirmation not required
- Decision: Treat a raised SQLSTATE from a GitHub lifecycle RPC as a first-class, client-safe status rather than an internal fault. `githubRouteErrorResponse` recognizes the PostgREST error shape — a plain object carrying `code` and `message`, not an `Error` instance — and maps only the allowlisted codes through the single table in `lib/server/http.ts` that the control-plane routes already share. Every other database fault keeps returning an opaque `500`, so schema and provider detail cannot leak. The allowlisted messages are authored by repository migrations, which is what makes returning them verbatim safe.
- Consequence: Three real refusals stopped lying to the owner. A disconnect against a changed installation returns `409 GitHub installation changed; reload before disconnecting` instead of a generic failure; re-synchronizing a terminally deleted installation id returns `409` explaining that a new provider installation is required; an installation already bound to another organization returns `403`. Goal item 19 — truthful Connected/Disconnected/Degraded/Error states — could not be satisfied while these surfaced as `500 internal_error`, because an owner cannot distinguish "we are broken" from "GitHub access changed underneath you".

## ADR-033 - Access loss is proven at the trusted write boundary, not in the UI

- Date: 2026-08-14
- Status: Accepted; behavioral proof complete, live confirmation pending two owner actions
- Decision: Prove every Phase 1B access-loss, disconnect, and reconnect claim by attempting a real change reservation through `reserve_github_change_request` after each transition, against the full `001`-`027` migration history applied to PGlite. A surface that renders "Not Connected" is not evidence; the only honest question is whether the database still permits a new GitHub write. Cover repository removal and re-add, archive and unarchive, repository deletion including a later stale event that must not resurrect it, installation suspension, an out-of-order unsuspend that must be ignored, a valid unsuspend, terminal installation deletion, owner disconnect, and reconnect/resync — each asserting the refusal or permission, that history and audit survive, and that the tenant's other installation is unaffected.
- Consequence: Items 11 through 15 move from code-only to behaviorally proven, and multi-installation isolation is demonstrated inside one tenant as well as across tenants. Two facts that only a boundary test surfaces are now locked in: a provider installation id can never be rebound to a second organization, and a disconnected installation can be resynchronized while a deleted one cannot. What remains is genuinely external — one second GitHub account/organization installation and one deliberate live adverse-event pass — so Phase 1B sits at 90% for want of a resource, not a design.
