...
**The current total is 41**, listed in the measured section below. The repository total is 105 migration
files; the hosted ledger's measured high-water mark is `20260814002300`, so everything after it is
outstanding. (Newest: `20260817000700_custom_pipeline_templates` — create/edit/delete for organization pipeline templates, three definer functions over the existing RLS table. Before it: `20260817000600_owner_operated_safety_controls` — the owner-operable kill switch and autonomy controls, ADR-080. Before it: `20260817000500_clear_all_finished_runs` — clear-all applied through the
