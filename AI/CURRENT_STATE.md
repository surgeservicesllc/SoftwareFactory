...
5. Repair the `supabase_migrations` ledger. It records 26 versions while 32 migrations are applied, so `supabase db push` would attempt to re-apply the last six and fail. Two files also share the version `20260813000200`, so the ledger can never distinguish them; resolving that needs an owner decision because `AI/ARCHITECTURE.md` records applied migration filenames as immutable.
