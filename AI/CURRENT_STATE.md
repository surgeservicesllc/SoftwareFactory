...
| Marketing content | Implemented locally and verified against in-process PostgreSQL; migration `20260813000100` unhosted | The schema, policies, grants and subscribe function pass a 21-assertion behavioral matrix as the real `anon`/`authenticated` roles. Pages render from the seeded fallback and are labelled **Demo Data** until the migration is applied to hosted Supabase. |
...
- Current working tree: lint and typecheck pass; full Vitest passes 50 files/434 tests; coverage passes at 73.18% statements, 70.44% branches, 71.90% functions, and 74.22% lines with required risk/constants thresholds satisfied (`lib/marketing` covers 97.61% and `lib/bots` 94.35% of statements); the full migration-chain RLS behavioral matrix passes 5/5 through migration `019`; the production build passes with the marketing and console route groups; and local Playwright passes 81/81 across desktop/tablet/mobile including axe checks on every marketing page, the bot fabric, and the console.
- The marketing behavioral matrix passes 21/21 against in-process PostgreSQL: the migration applies with its seed under the sensitive-data guard, RLS and FORCE RLS hold on all eleven tables, anonymous readers see published rows only, both browser roles are refused INSERT/UPDATE/DELETE on every content table, neither can read `newsletter_subscribers`, and the subscribe function normalizes, is idempotent, answers identically for a repeat address, rejects malformed input, and falls back to a safe source. Running it caught a real defect a textual assertion could not: `marketing_plan_features.label` was `NOT NULL` while matrix-only rows legitimately carry no card label; that column is now nullable.
