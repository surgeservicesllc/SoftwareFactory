...
- `tests/integration/github-lifecycle-matrix.test.ts` applies the full `001`-`027` history to PGlite and drives two independent installations in one tenant plus a third in a second tenant through the complete access-loss matrix. It proves at the trusted write boundary that repository removal, archival, deletion, installation suspension, terminal installation deletion, and owner disconnect each refuse new GitHub work, that history and audit survive every one of them, that reconnect/resync creates no duplicate or corrupt record, that a provider installation id cannot be rebound to a second organization, and that owner-allowed/unrelated-denied/anonymous-denied holds throughout. Raw `activity_events` and `github_webhook_deliveries` are unreadable by any authenticated caller; Activity is served only through the caller-scoped `list_activity` RPC.
...
2. Install the App on a second, independent GitHub account or organization, and run one deliberate live adverse-event pass (repository remove/re-add, suspend, unsuspend, uninstall) on that throwaway installation only. These are the two remaining Phase 1B gaps; exact steps and verification are in `AI/PHASE_1B_COMPLETION.md`. The adverse matrix itself is already proven behaviorally against the real migrations, so this is live confirmation rather than unbuilt behavior.
