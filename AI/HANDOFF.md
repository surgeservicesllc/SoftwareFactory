...
Finish Phase 1B end to end. A post-review hardening set now exists in the local working tree: active-organization enforcement across GitHub routes, truthful live connection/project/file state, a live tenant Activity stream, removal of the local HTTP writer, stronger provider response/secret/path validation, actor-attributed terminal change evidence, and signed-webhook repository-grant reconciliation. Hosted Supabase remains through `010`, and the verified READY Vercel deployment predates this work. Hosted migrations `011`-`013`, current-tree gates/deployment, authenticated tenant behavior, the in-product GitHub callback/connection, webhook delivery, and remaining production acceptance are incomplete. Do not describe GitHub as Connected or Phase 1B as complete yet.
...
2. Push/deploy the exact reviewed hardening commit and record its new Vercel deployment identity; do not reuse the prior runtime's validation as evidence for it.
3. Verify hosted cross-tenant/anonymous denial, privileged RPC authorization, audit immutability/redaction, and a real authenticated application session.
4. Complete production Supabase Auth/onboarding.
5. Complete the authenticated owner callback for existing provider installation `153286187`, persist/sync the tenant connection, link the project, inspect live repository state, create one safe controlled draft PR, and test disconnect/loss paths.
6. Configure/verify the blank/inactive GitHub webhook endpoint, then observe signed webhook/audit and newly granted repository reconciliation.
7. Update this memory/scorecard with exact evidence; only then issue the Phase 1B completion report.
