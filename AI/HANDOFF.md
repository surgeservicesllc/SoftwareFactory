...
Finish the Phase 1C provider-credit recovery and live acceptance without overstating status. The protected Supabase ledger reconciliation and forward-only chain through `130014` are complete on exact project `qpuofpmagrmyamahqwxw`; linked lint and focused runtime/catalog/ACL checks pass. The fail-closed provider-startup recovery is published on `main` at `bc95b9e3a5952864bd26da778a052f37400ea747`; CI run `31748567790` passed both required jobs and matching Vercel deployment `dpl_3hTUZ1aJy2b2BSdhTZMnZRMfxnhh` is READY. Distinct no-claim diagnostic Actions run `31748582858` passed the exact-model GET, then the bounded non-stored Responses probe returned the safe code `credit_balance_exhausted`; Docker preload and durable claim were skipped. Durable run `f4594556-6f72-4763-a480-6993939e3651` remains failed after attempt 1 of 2, retryable, with its final retry unconsumed. Activation is absent/OFF. The user-pasted OpenAI key is treated as compromised and `SOFTWAREFACTORY_OPENAI_API_KEY` has been removed from repository Actions secrets; it must stay absent until a fresh funded replacement is available. No successful Phase 1C run or draft PR exists, so Phase 1C remains **Not Connected**. All automatic actions remain OFF and the global kill switch remains ON.
...
`SOFTWAREFACTORY_OPENAI_API_KEY` is intentionally absent. The value pasted into chat must be treated as compromised and revoked at the provider; do not restore it. Keep the repository secret absent until the owner supplies a fresh key for a project with available credits, then configure only that replacement through the protected secret path.
...
- Hosted history is reconciled through `130014`, and linked lint is clean. Recovery commit `bc95b9e3a5952864bd26da778a052f37400ea747` is on `main`; CI run `31748567790` passed both required jobs and matching Vercel deployment `dpl_3hTUZ1aJy2b2BSdhTZMnZRMfxnhh` is READY.
...
- Published provider-only diagnostic `31748582858` skipped Docker preload and durable claim. The exact-model GET passed; the bounded non-stored Responses call returned only the safe machine-readable code `credit_balance_exhausted`. Attempt 2 remains unconsumed and activation is OFF.
- The recovery patch is published. The exact blocker is funding: add credits to the OpenAI project or supply a fresh key for a funded replacement project, then rerun the no-claim diagnostic before using the final durable retry. No production-monitor journey or successful live Phase 1C provider result exists.
...
2. Revoke the user-pasted OpenAI key at the provider. Keep `SOFTWAREFACTORY_OPENAI_API_KEY` absent and activation OFF while adding credits or obtaining a fresh funded replacement project key.
3. Configure only the fresh funded replacement key through the protected repository-secret path.
4. Within the approved window, admit only the distinct `softwarefactory_phase1c_preflight` event, then return activation to absent/OFF. Require the pinned CLI, exact-model GET, and bounded non-stored Responses probe to pass while Docker preload and durable claim remain skipped.
5. If that diagnostic fails or is ambiguous, stop; attempt 2 must remain unconsumed. Only after it passes, re-enable briefly and use the production Retry control for durable run `f4594556-6f72-4763-a480-6993939e3651`. This is its second and final allowed attempt.
