...
- The marketing schema is verified by `tests/integration/marketing-rls-behavior.test.ts`, which applies the migration to in-process PostgreSQL and exercises it as the real `anon`/`authenticated` roles (21/21). Treat that file as the guard on this boundary; do not weaken a marketing grant or policy without it failing first.
...
- Current tree: lint/typecheck pass; full Vitest passes 50 files/434 tests; full-chain RLS behavior passes 5/5 through migration `019`; production build passes with 40 routes.
