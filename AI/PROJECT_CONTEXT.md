...
This implementation is not yet a Connected worker capability. Hosted Supabase remains current only through migration `027`; Phase 1E migration `028` and the pending provider/synthetic-journey/bot-fabric/marketing/Phase 1C chain `130001` through `130008` are unapplied. The Phase 1C GitHub Actions secrets are not configured, the fail-closed `SOFTWAREFACTORY_PHASE1C_WORKER_ENABLED` repository variable is OFF/unverified, the worker has not produced a live heartbeat, and no real Codex run has completed. OpenAI/Codex therefore remains **Not Connected**.
