...
| 4 | Dispatch a REAL task through the zero-token path | **PASS** | **Closed 2026-08-15.** Previously partial because the transport's *credential wiring* was never exercised — this environment has only an ambient credential the transport deliberately refuses. The graph live canary [run 31899176257](https://github.com/surgeservicesllc/SoftwareFactory/actions/runs/31899176257) runs in Actions where `SOFTWAREFACTORY_CLAUDE_CODE_OAUTH_TOKEN` is configured, and its log states which path it took: `executing through the Phase 2A provider transport (subscription credential configured)`. Five real Claude nodes, 244s, zero API tokens. |
