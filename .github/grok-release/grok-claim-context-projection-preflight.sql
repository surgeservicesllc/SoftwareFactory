...
select pg_catalog.set_config(
  'softwarefactory.grok_claim_context_release.operation', :'operation', false
) as operation_setting \gset
select pg_catalog.set_config(
  'softwarefactory.grok_claim_context_release.unrelated_ledger_sha256',
  :'unrelated_ledger_sha256', false
) as unrelated_ledger_setting \gset

do $grok_claim_context_release_preflight$
declare
  v_operation text := pg_catalog.current_setting(
    'softwarefactory.grok_claim_context_release.operation'
  );
...
begin
  if v_operation not in ('probe', 'apply', 'verify') then
    raise exception 'grok_claim_context_release_operation_invalid';
  end if;
  ...
  then
    raise exception 'grok_claim_context_release_database_identity_mismatch';
  end if;
  ...
  then
    raise exception 'grok_claim_context_release_prerequisite_ledger_or_catalog_mismatch';
  end if;
  ...
  if v_unrelated_ledger_sha256 is distinct from pg_catalog.current_setting(
    ...
    raise exception 'grok_claim_context_release_unrelated_ledger_changed';
  end if;

  select pg_catalog.count(*)::integer
    into v_ledger_count
    from supabase_migrations.schema_migrations
   where version = '20260831001500';
  if v_operation in ('probe', 'apply') then
    ...
    then
      raise exception 'grok_claim_context_release_absent_ledger_or_catalog_mismatch';
    end if;
  elsif v_ledger_count <> 1 then
    raise exception 'grok_claim_context_release_verify_ledger_mismatch';
  end if;

  for v_expected in
    select * from (values
      ('public.grok_initial_context_claim_projection(uuid)',
       case when v_operation in ('probe','apply') then null else '06c7fb24b7c4b50bbf80aee57385ff57' end,
       false),
      ('public.attach_current_grok_admissions_to_claim(jsonb)',
       case when v_operation in ('probe','apply') then '427294dac55a06b5ca1f9a1c89b0cdfa' else 'c1075dafaa5bc957d16ff2599382a811' end,
       false),
      ('public.attach_current_grok_admission_to_phase1c_claim(jsonb)',
       case when v_operation in ('probe','apply') then '48f84aba274b2e6316331de8e3fca796' else '2562fa378097239ce4a3e47e9121d410' end,
       false),
  ...
  loop
    ...
    if v_expected.source_md5 is null then
      if v_routine is not null then
        raise exception 'grok_claim_context_release_unexpected_function: %', v_expected.signature;
      end if;
    elsif v_routine is null or not exists (
      ...
      raise exception 'grok_claim_context_release_function_identity_mismatch: %', v_expected.signature;
    end if;
  end loop;
  ...
  if exists (
    ...
    raise exception 'grok_claim_context_release_safety_state_not_stopped';
  end if;
end;
$grok_claim_context_release_preflight$;

select 'grok-claim-context-release-preflight-ok';
