...
describe("summarising a run by stage", () => {
  ...
  it("returns stages in lifecycle order, not the order nodes arrived", () => {
    ...
    const { stages } = summariseRunStages([
      node("MONITOR", "COMPLETED"),
      node("REQUIREMENT", "COMPLETED"),
      node("TEST", "COMPLETED"),
    ]);

    expect(stages.map((entry) => entry.stage)).toEqual(["REQUIREMENT", "TEST", "MONITOR"]);
  });
  ...
  it("omits a stage the run never contained rather than zeroing it", () => {
     ...
     * "DEPLOY 0/0" would invent a stage the graph was never going to enter,
     * which is the fake-progress the goal forbids.
     *
     * The name has to be one the vocabulary actually holds. This case named
     * `DEPLOYMENT` — an eight-stage label ADR-137 removed — and an unrecognised
     * stage is absent from the result for a different reason entirely, so the
     * assertion passed while testing nothing.
     */
    const { stages } = summariseRunStages([node("REVIEW", "COMPLETED")]);

    expect(stages).toHaveLength(1);
    expect(SDLC_STAGES).toContain("DEPLOY");
    expect(stages.map((entry) => entry.stage)).not.toContain("DEPLOY");
  });
