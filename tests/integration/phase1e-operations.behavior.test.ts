...
describe("Phase 1E production operations behavior", () => {
  ...
  it("keeps every operations table under RLS with FORCE RLS and no service_role table grants", async () => {
    ...
    // session/evidence tables; and 198 since recurring billing (ADR-200)
    // added crm_billing_runs and crm_dunning_notices. Each is RLS-enabled
    // and forced, which the filter on the next line is what actually
    // proves.
    expect(rlsRows).toHaveLength(198);
