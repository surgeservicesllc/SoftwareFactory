...
describe("Phase 1E production operations behavior", () => {
  ...
  it("keeps every operations table under RLS with FORCE RLS and no service_role table grants", async () => {
    ...
    // tables, the Phase 1D decision audit, the three Phase 2C resource tables,
    ...
    // (ADR-198) added crm_portal_users and crm_portal_requests; 196 with
    // main's durable Grok workspace and its seven tenant-scoped
    ...
    // integration registry (ADR-207) added crm_service_integrations; 204
    // since Grok provider admission (ADR-208) added the immutable
    ...
    // multi-unit properties (ADR-215) added crm_property_units; 209
    // since filed service documents (ADR-216) added crm_service_documents;
    ...
    // context tables; 223 after later Grok completion evidence; and 226 after
    // ADR-239 added the three append-only initial Resume wake evidence tables.
    // (Both earlier branches counted 215 -> 217 independently; their union
    // was 219.)
