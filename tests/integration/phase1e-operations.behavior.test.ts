...
describe("Phase 1E production operations behavior", () => {
  ...
  it("keeps every operations table under RLS with FORCE RLS and no service_role table grants", async () => {
    ...
    // resume-extraction table, the job-seeker document-review table, and the
    // denied transaction-local factory record-only submission guard table.
    // The filter below is the real guarantee — this count exists so a new
    // table cannot slip in unexamined.
    expect(rlsRows).toHaveLength(131);
