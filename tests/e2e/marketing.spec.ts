...
const MARKETING_ROUTES = [
  ...
  { path: "/solutions", heading: /see the factory/i },
...
test("navigates from the home page to Solutions", async ({ page }, testInfo) => {
  await page.goto("/");
  await openNavigation(page, testInfo.project.name);

  await page.getByRole("link", { name: "Solutions", exact: true }).first().click();
  await expect(page).toHaveURL(/\/solutions$/);
  await expect(page.getByRole("heading", { level: 1 })).toContainText(/see the factory/i);
});
