/**
 * The secret-box implementation, deliberately without the `server-only` guard.
 *
 * Two runtimes need this code: the Next.js server (routes seal credentials
 * and relay codes) and the auth-broker worker, which runs under plain Node in
 * GitHub Actions — where importing `server-only` throws by design. The guard
 * still protects application code: `lib/server/secret-box.ts` re-exports this
 * module behind `import "server-only"`, and that is the path everything under
 * `app/` and `lib/server/` imports. Import this core directly only from code
 * that runs outside Next entirely (`lib/worker/`, `scripts/`).
 */
import {
  createCipheriv,
  createDecipheriv,
