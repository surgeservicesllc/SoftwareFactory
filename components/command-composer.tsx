...
import { postJson } from "@/lib/client/use-tenant-resource";
...
const examples = [
  ...
  "Get this project production ready",
] as const;
...
type CommandResponse = {
  ...
  execution?: { started: boolean; message: string };
...
type SubmissionState =
  | { kind: "idle" }
  | { kind: "pending" }
  | { kind: "success"; response: CommandResponse }
  | { kind: "error"; message: string };

type ProjectOption = { id: string; name: string; status: string; connectionStatusLabel?: string };
...
export function CommandComposer({
  ...
  async function submitCommand(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!projectId) {
      setState({ kind: "error", message: "Choose a connected project before submitting this command." });
      return;
    }

    setState({ kind: "pending" });
    try {
      const { ok, body } = await postJson<CommandResponse>("/api/commands", {
        prompt: trimmed,
        risk: riskLevel.toLowerCase(),
        projectId,
        parameters: {},
      });
      if (!ok) throw new Error(body.error?.message ?? "The command could not be saved.");
    ...
    <form onSubmit={submitCommand} className="panel panel-grid overflow-hidden rounded-xl">
      <div className="border-b border-[#26313e] px-4 py-3 sm:px-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-[9px] font-semibold uppercase tracking-[0.15em] text-[#819164]">
              Command intake
            </span>
          </div>
          <span className="inline-flex items-center gap-1.5 text-[10px] text-[#687586]">
            <span
              className={cn(
                ...
                workerStatusLabel === "Connected" ? "bg-[#c6f135]" : "bg-[#ff6b72]",
              )}
              aria-hidden="true"
            />
      ...
      <div className="p-4 sm:p-5">
        <label className="mb-3 block">
          <span className="mb-2 block font-mono text-[9px] font-semibold uppercase tracking-[0.13em] text-[#788596]">
            Project context
          </span>
          <select
            value={projectId}
        ...
        <label htmlFor="bot-command" className="sr-only">
          Tell the Bot Manager what to do
        </label>
        <textarea
          id="bot-command"
          value={instruction}
        ...
        <p id="command-help" className="mt-1 px-1 text-[10px] leading-5 text-[#637081]">
          The orchestrator classifies intent and risk, then persists a durable plan. A queued run is
          recorded intent and evidence — never proof that work succeeded.
        </p>
        ...
        <div className="mt-5 flex flex-col gap-3 border-t border-[#202a36] pt-4 sm:flex-row sm:items-end sm:justify-between">
          <fieldset>
            <legend className="mb-2 font-mono text-[9px] uppercase tracking-[0.13em] text-[#647182]">
              Declared minimum risk
            </legend>
          ...
          >
            ...
            {state.kind === "pending" ? "Planning…" : "Submit command"}
          </button>
        </div>
        ...
        <div id="command-status" aria-live="polite" className="mt-4">
          {riskLevel === "RED" && state.kind === "idle" ? (
            <p className="flex items-start gap-2 rounded-lg border border-[#502c31] bg-[#2b181c] p-3 text-[10px] leading-5 text-[#e59399]">
              ...
              RED work is recorded and planned, but a run never starts without an explicit owner approval.
            </p>
          ) : null}

          {state.kind === "success" ? (
            <div className="rounded-lg border border-[#36491d] bg-[#18220f] p-3.5">
              <p className="flex items-start gap-2 text-[10px] leading-5 text-[#c5dd77]">
                <CheckCircle2 className="mt-0.5 size-3.5 shrink-0 text-[#c6f135]" aria-hidden="true" />
                {state.response.execution?.message ?? "The command was persisted."}
              </p>
              {state.response.plan ? (
                <div className="mt-3 border-t border-[#2c3a1a] pt-3">
                  <p className="font-mono text-[9px] uppercase tracking-[0.12em] text-[#8fa356]">
                    Plan · {state.response.plan.intent.replace(/_/g, " ")} ·{" "}
                    {state.response.plan.risk.toUpperCase()}
                  </p>
                  <ol className="mt-2 space-y-1.5">
                    {state.response.plan.tasks.map((task, index) => (
                      <li key={task.title} className="flex gap-2 text-[10px] leading-4 text-[#a9b78a]">
                        <span className="font-mono text-[#7c8e4f]">{index + 1}.</span>
                        <span>
                          {task.title}
                          <span className="text-[#6f7d51]">
                            {" "}
                            · {task.agentRole} · {task.workType.replace(/_/g, " ")}
