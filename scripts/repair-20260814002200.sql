-- Corrective repair for 20260814002200_graph_anchors.
--
-- That migration applied partially against hosted. Its enum `anchor_kind`
-- exists, so re-running the original file fails at its first statement with
-- `42710: type "anchor_kind" already exists` -- before reaching the four tables,
-- the policies, the functions or the `graph_verifications.anchor_count` column.
--
-- Same class of problem as 20260814000210, and the same remedy: the migration
-- made idempotent so it can run against ANY partial state, including the
-- fully-applied one where it is a no-op. Verified both ways against real
-- PostgreSQL before being written here.
--
...
--
-- `create or replace function`, the RLS statements, the grants and the comments
-- are already idempotent and are untouched.
--
-- PostgreSQL has no `create type if not exists`, which is why the enums are
-- guarded by exception rather than by a clause. The handler catches
-- duplicate_object only, so a genuine error in the type definition still fails
-- loudly rather than being swallowed.
--
-- No RLS is disabled, no data is deleted, and no grant is widened.
--
-- RUN THIS INSIDE THE TRANSACTION BELOW. The Supabase SQL editor does not wrap
-- statements for you, and a failure partway is what produced this state.
...
create table if not exists public.graph_anchors (
...
create index if not exists graph_anchors_run_idx on public.graph_anchors (graph_run_id, observed_at desc);
create index if not exists graph_anchors_node_run_idx on public.graph_anchors (node_run_id)
...
create table if not exists public.node_run_claims (
  id uuid primary key default gen_random_uuid(),
  organization_id uuid not null references public.organizations(id) on delete cascade,
...
create index if not exists node_run_claims_node_run_idx on public.node_run_claims (node_run_id);
...
create table if not exists public.claim_anchors (
  organization_id uuid not null references public.organizations(id) on delete cascade,
...
create index if not exists claim_anchors_anchor_idx on public.claim_anchors (anchor_id);
...
create table if not exists public.claim_acceptable_anchors (
  claim public.anchored_claim not null,
  kind public.anchor_kind not null,
...
drop policy if exists graph_anchors_member_select on public.graph_anchors;
create policy graph_anchors_member_select on public.graph_anchors
  for select using (public.is_organization_member(organization_id));

drop policy if exists node_run_claims_member_select on public.node_run_claims;
create policy node_run_claims_member_select on public.node_run_claims
  for select using (public.is_organization_member(organization_id));

drop policy if exists claim_anchors_member_select on public.claim_anchors;
...
drop policy if exists claim_acceptable_anchors_read on public.claim_acceptable_anchors;
...
alter table public.graph_verifications
  add column if not exists anchor_count integer not null default 0 check (anchor_count >= 0);
...
-- Verify. Expect four rows, every rls column reading `rls+force`, and
-- anchor_count present on graph_verifications.
...
select count(*) as anchor_count_column
  from information_schema.columns
 where table_schema = 'public' and table_name = 'graph_verifications' and column_name = 'anchor_count';
