...
| `027_handoff_github_project_connection` | Owner-only atomic handoff between two active installations for the same GitHub account/external repository; requires signed processed target-delivery evidence, preserves history, serializes change reservations, audits the transition, and supports evidence-bound reverse handoff | Local only; not applied to hosted Supabase |
...
The hosted ledger is current through `026`; the repository now contains local migration `027`, so it intentionally leads hosted state by one unapplied forward migration. The pre-`027` local/remote history matched, linked dry run was up to date, and lint was clean. Catalog verification reports 23/23 public tables with RLS and FORCE RLS, 32 policies, zero policyless tables, 22 secret guards, and false tested raw authenticated/browser grants.
...
Migration `027` is an additive pre-release handoff boundary. It replaces the change-request binding trigger with a transaction-locking version and adds `handoff_github_project_connection`. The RPC requires the authenticated active organization owner, exact source/target installation and repository identifiers, two distinct live connections/installations for the same provider account, selected/non-archived/non-disabled matching repositories, exactly one primary project link, no conflicting active link, and no reserved change request. A first handoff also requires a processed webhook delivery for the exact target connection and installation. It preserves the existing project UUID and change/audit history, updates only the project connection/repository binding plus synchronized display metadata, and appends immutable `project_connection_handoff` evidence. A reverse handoff may rely on that prior immutable event while both installations remain active. It must not be described as hosted until application, ledger, lint, RLS/grant, callback, signed-delivery, and live handoff checks pass.
