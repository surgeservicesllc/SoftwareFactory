...
Current stop condition: do not call the hosted data boundary fully verified until database login-role authorization is restored, migrations `011`-`025` are exactly approved/applied, and authenticated cross-tenant, anonymous-denial, privileged-RPC, audit, and real application-session checks pass. The hosted ledger remains through `010`; linked lint is clean, while the successful dry run covered only `011`-`024` and the current full-chain attempt returns `403`.
