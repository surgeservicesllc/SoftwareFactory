...
create table public.resource_breakers (
...
create table public.resource_breaker_events (
  id uuid primary key default gen_random_uuid(),
  organization_id uuid not null references public.organizations(id) on delete cascade,
...
create index resource_breaker_events_recent
...
create index resource_assignments_project_recent
  on public.resource_assignments (project_id, decided_at desc);
create index resource_assignments_node
