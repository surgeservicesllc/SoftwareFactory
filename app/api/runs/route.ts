...
import { listProviderRuns, loadProjectRoutingContext } from "@/lib/providers/service";
...
export async function GET(request: Request) {
  try {
    ...
    const limit = limitParameter && /^\d{1,3}$/.test(limitParameter) ? Number(limitParameter) : 25;

    return jsonNoStore({ runs: await listProviderRuns(client, activeOrganization.id, limit) });
  } catch (error) {
    const boundary = supabaseBoundaryErrorResponse(error);
    if (boundary) return boundary;

    return jsonNoStore(
      { error: { code: "internal_error", message: "Runs could not be loaded." } },
      { status: 500 },
    );
  }
}
...
/**
 * Execute one advisory provider run.
 *
 * The run reads context and returns a structured artifact. It has no
 * repository, merge, deployment, or approval authority, and it is gated on an
 * owner having switched outbound provider execution on.
 */
