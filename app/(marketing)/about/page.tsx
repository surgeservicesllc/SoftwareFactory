      ...
      <MarketingSection className="mt-10">
        <SurfacePanel className="grid grid-cols-1 gap-10 p-6 sm:p-8 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,2fr)]">
          <div className="lg:border-r lg:border-[#1c2433] lg:pr-8">
            ...
            <div className="mt-6 flex items-center gap-2 text-sm text-[#7f8c9e]">
            ...
            <ul className="mt-6 grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-5">
              {values.map((value) => (
                <li key={value.title}>
                  ...
                  <p className="mt-2 text-sm leading-5 text-[#7f8c9e]">{value.body}</p>
                </li>
              ))}
            </ul>
          </div>
            ...
            <li key={member.name}>
              <SurfacePanel className="flex h-full overflow-hidden">
                ...
                <div className="flex min-w-0 flex-1 flex-col p-4">
                  ...
                  <p className="mt-1 text-sm text-[#7f8c9e]">{member.role}</p>
                  <div className="mt-3 flex gap-2">
                    {member.linkedinUrl ? (
                      <a
                        ...
                        className="grid size-7 place-items-center rounded-md border border-[#25303f] font-mono text-xs font-bold lowercase text-[#60a5fa] transition-colors hover:border-[#3f5570]"
                      >
                    ...
                    {member.twitterUrl ? (
                      <a
                        ...
                        className="grid size-7 place-items-center rounded-md border border-[#25303f] font-mono text-sm font-bold text-[#60a5fa] transition-colors hover:border-[#3f5570]"
                      >
                        <span aria-hidden="true">X</span>
                      </a>
                    ) : null}
                  </div>
                  <p className="mt-3 text-xs leading-4 text-[#7f8c9e]">{member.bio}</p>
                </div>
              </SurfacePanel>
            </li>
          ))}
