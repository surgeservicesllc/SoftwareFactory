...
export default async function AboutPage() {
  ...
  return (
    <>
        ...
        <SurfacePanel className="grid gap-10 p-6 sm:p-8 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,2fr)]">
          <div className="lg:border-r lg:border-[#1c2433] lg:pr-8">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-lg font-semibold tracking-[-0.02em] text-white">Our Mission</h2>
            </div>
            <p className="mt-5 text-sm leading-7 text-[#8593a5]">
            ...
            <div className="mt-6 flex items-center gap-2 text-[11px] text-[#7f8c9e]">
              ...
              {values.map((value) => (
                <li key={value.title}>
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-[13px] font-semibold text-white">{value.title}</h3>
                  </div>
                  <p className="mt-2 text-[11px] leading-5 text-[#7f8c9e]">{value.body}</p>
                </li>
              ))}
            </ul>
          </div>
            ...
            <li key={member.name}>
              <SurfacePanel className="flex h-full overflow-hidden">
                ...
                <div className="flex min-w-0 flex-1 flex-col p-4">
                  ...
                  <p className="mt-1 text-[11px] text-[#7f8c9e]">{member.role}</p>
                  <div className="mt-3 flex gap-2">
                    {member.linkedinUrl ? (
                      <a
                        ...
                        className="grid size-7 place-items-center rounded-md border border-[#25303f] font-mono text-[10px] font-bold lowercase text-[#60a5fa] transition-colors hover:border-[#3f5570]"
                      >
                    ...
                    {member.twitterUrl ? (
                      <a
                        ...
                        className="grid size-7 place-items-center rounded-md border border-[#25303f] font-mono text-[11px] font-bold text-[#60a5fa] transition-colors hover:border-[#3f5570]"
                      >
                        <span aria-hidden="true">X</span>
                      </a>
                    ) : null}
                  </div>
                  <p className="mt-3 text-[10px] leading-4 text-[#7f8c9e]">{member.bio}</p>
                </div>
              </SurfacePanel>
            </li>
          ))}
      ...
      <MarketingSection className="mt-10">
        <SurfacePanel className="flex flex-col items-center gap-6 px-6 py-6 lg:flex-row lg:gap-10">
          <div className="flex shrink-0 items-center gap-3">
            ...
            <p className="text-sm font-medium text-[#c8d2df]">Trusted by innovative teams worldwide</p>
          </div>
          <ul className="flex flex-1 flex-wrap items-center justify-center gap-x-10 gap-y-4 lg:justify-between">
            {content.logos.map((logo) => (
              <li
                key={logo.name}
                className="text-[17px] font-semibold tracking-[-0.01em] text-[#8b97a8] transition-colors hover:text-[#c8d2df]"
              >
                {logo.wordmark}
              </li>
            ))}
      ...
      <MarketingSection className="mt-10">
        <div className="flex items-center gap-3 rounded-xl border border-[#1c2433] bg-[#0b0f18] px-4 py-3 text-[11px] leading-5 text-[#7f8c9e]">
