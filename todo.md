...
## GRAPH — ADR-136 IS SUPERSEDED: THE ENUM DOES GROW, BECAUSE SOMETHING NOW POPULATES IT (2026-08-23, round 6 — PICK UP HERE)
...
runs at all**. Not a failing run, not an error: no run object. That is already
written down further down this file (2026-08-19) and it caught me again anyway,
because the symptom is silence and silence reads like "still queued". Three
pushes landed with no CI created before I checked mergeability.

Two things make it cheap to catch. `git merge-tree --write-tree --name-only
HEAD origin/main` exits non-zero and names the conflicting files; the older
two-argument `git merge-tree` form does **not** report conflicts the same way
and will tell you the tree is clean when it is not — I read it wrong once here.
And the pull request's own `mergeable_state` says `dirty` long before anything
else does. Check one of those before assuming CI is slow.
...
## GRAPH — THE BACKFILL IS APPLIED IN PRODUCTION, AND THE STAGE VOCABULARY WAS SETTLED AT EIGHT (2026-08-23, round 4 — vocabulary part superseded by round 6 above)
...
> Item 1 below is superseded by round 6: build the ten, not the eight. Items
> 2–4 still stand.
